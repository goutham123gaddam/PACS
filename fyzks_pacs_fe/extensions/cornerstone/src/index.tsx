...
import CardioThoracicRatio from './utils/measurementServiceMappings/CardioThoracicRatio';
import CardioThoracicRatioTool from './tools/CTRTool';
import SpineLabeling from './utils/measurementServiceMappings/SpineLabelling';
...
const cornerstoneExtension: Types.Extensions.Extension = {
  ...
  preRegistration: async function (props: Types.Extensions.ExtensionParams): Promise<void> {
    ...
    // Add CTR value type
    measurementService.addValueType({
      CARDIO_THORACIC_RATIO: 'value_type::cardioThoracicRatio',
    });

    // Add CTR schema keys
    measurementService.addMeasurementSchemaKeys([
      'cardiacWidth',
      'thoracicWidth',
      'cardioThoracicRatio',
    ]);

    // Add measurement mapping for CTR
    const source = measurementService.getSource('cornerstone') ||
                  measurementService.createSource('cornerstone', '1');

    measurementService.addMapping(
      source,
      'CardioThoracicRatio',
      {
        annotationType: 'CardioThoracicRatio',
      },
      CardioThoracicRatio.toAnnotation,
      CardioThoracicRatio.toMeasurement
    );
